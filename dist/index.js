...
;// CONCATENATED MODULE: ./node_modules/@tarosky/farmhand/src/parser/index.js
/**
 ...
 * @returns {number}
 */
function fermentationDays( date ) {
 ...
 */
function extractMajorWpVersion( version ) {
 ...
 * @return {boolean}
 */
function wpVersionFresh ( version, latest, acceptable = 3 ) {
 ...
 */
function versionCompare( version, target ) {
...
;// CONCATENATED MODULE: ./node_modules/@tarosky/farmhand/src/request/index.js
/**
 ...
 */
function getRepos( tag, owner, isOrganization = false ) {
    let query = 'topic:' + tag;
    if ( isOrganization ) {
        query += '+org:' + owner;
    } else {
 ...
 */
function getRepoFile ( repo_name, path ) {
 ...
 * @returns {Promise<string>}
 */
function wpVersionStatus( version ){
 ...
 */
function isWpVersionSecure ( version ) {
...
async function run() {
  try {
    ...
    const readmeFile = await external_fs_.promises.readFile( readme, 'utf8' );
